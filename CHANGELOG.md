...
* protocols/rpc: Add optional `minLedger` and `maxLedger` bounds to `GetTransactionRequest`, restricting a `getTransaction` lookup to that inclusive ledger range, plus `GetTransactionRequest.IsValid` ([#6009](https://github.com/stellar/go-stellar-sdk/pull/6009))
* rpcclient: Add `PollTransactionOptions.WithMinLedger`, which bounds every poll to ledgers at or after the given sequence ([#6009](https://github.com/stellar/go-stellar-sdk/pull/6009))
