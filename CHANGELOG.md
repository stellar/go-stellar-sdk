...
* protocols/rpc: Add optional `minLedger` and `maxLedger` bounds to `GetTransactionRequest`, restricting a `getTransaction` lookup to that inclusive ledger range, plus `GetTransactionRequest.IsValid`.
* rpcclient: Add `PollTransactionOptions.WithMinLedger`, which bounds every poll to ledgers at or after the given sequence.
