...
### New Features
* Add `BufferedStorageBackendConfig.BufferBytes`, a byte cap on download prefetch, converted into a queue depth — object size varies ~700x across pubnet history, so `BufferSize` alone says nothing about memory. Zero (the default) disables it. The compressed-buffer pool is now sized by `NumWorkers` rather than `BufferSize`. Peak RSS on a pubnet tip backfill: 25.0 GB to 5.8 GB. [5988](https://github.com/stellar/go-stellar-sdk/pull/5988), [stellar-rpc#895](https://github.com/stellar/stellar-rpc/issues/895).
...
* Add the zero-copy `LedgerTransactionView` read path (**experimental** — signatures may still change), reading directly off the generated `*View` accessors without `UnmarshalBinary`: `LedgerTransactionViewByHash` / `LedgerTransactionViewRange` (the view parallel of `LedgerTransaction` / `LedgerTransactionReader`; these pair TxSet envelopes by hash and take the network passphrase). `network.TransactionViewHasher` computes transaction hashes straight from envelope view bytes. Validated wire-identical against the parsed path across LCM V0/V1/V2 and `TransactionMeta` V0–V4, plus real-ledger equivalence tests and benchmarks against the eager-decode path on `ledger_58752000`. Per-operation event **arity** matches `LedgerTransaction.GetTransactionEvents` as well: a `TransactionMeta` V3 Soroban transaction reports exactly one operation slot even when `SorobanMeta` is absent — the shape of a Soroban transaction that was charged but never executed, seen on pubnet under protocols 20–22 — where `ContractEvents` previously came back with no operation slots at all. [5949](https://github.com/stellar/go-stellar-sdk/pull/5949), [5997](https://github.com/stellar/go-stellar-sdk/pull/5997).
* Add the transaction byte-span entry points to the zero-copy view API (**experimental** — signatures may still change), for consumers that record where each transaction lives in a ledger once and serve it later without walking the ledger again. `LedgerTxParts` now carries `ElemStart`/`ElemEnd`, the byte offsets of that transaction's whole `TxProcessing` element within the `xdr.LedgerCloseMetaView` it was extracted from — free, because the walk already sizes every element in order to advance, and consecutive elements tile the array. `ExtractLedgerTxEnvelopeSpans` is the TxSet-side counterpart: one `TxEnvelopeSpan{Hash, Start, End}` per envelope in agreed-set order, every envelope hashed with `network.TransactionViewHasher` so a caller pairs the two sides by hash (agreed-set order is not apply order). `LedgerTransactionViewFromParts` then rebuilds a full `LedgerTransactionView` from those two byte slices plus the `LedgerCloseMeta` version, the apply index and the ledger header fields, with no ledger in hand; its byte fields alias the caller's input, the same zero-copy contract as the rest of the view path. It is the per-element half of `LedgerTransactionViewByHash` lifted out — that path and `LedgerTransactionViewRange` now run the same collect/resolve/assemble code, so the three cannot drift. Pairing is deliberately the caller's (no hash is recomputed, hence no passphrase), while malformed element or envelope bytes are an error rather than a panic. Covered across LCM V0/V1/V2 × `TransactionMeta` V0–V4 × fee-bump, Soroban, multi-phase and parallel-phase TxSets — including TxSets listed in the reverse of apply order — plus real-pubnet-ledger equivalence against `LedgerTransactionViewByHash`.

### Performance Improvements
* The zero-copy view extractors size each `TransactionMeta` V4 operation exactly once. The per-operation collection now steps the operations array by hand — `Events` is the LAST `OperationMetaV2` field, so one locate plus the events array's own extent positions the next operation — where the generated `Iter()` sized every operation's whole interior just to advance and the events accessor then re-sized its `Changes`. Event arrays are collected the same way, trimming each element once instead of sizing it once to advance and again to trim, into slices presized from the element count; `ExtractLedgerTxParts` likewise presizes its output from the `TxProcessing` count prefix, an O(1) read. The products are unchanged, byte for byte and offset for offset. On a dense synthetic ledger (6,000 transactions, ~10 contract events per operation, `TransactionMeta` V3 and V4), `ExtractLedgerTxParts` + `EventsFromTxParts` goes from 12.4ms/36.0k allocs to 8.2ms/16.0k allocs per ledger, and all three products together from 14.0ms to 10.1ms; on the `ledger_58752000` pubnet fixture (249 transactions, all V3 metas, where the V4 spine never runs) `LedgerTransactionViewRange` over the whole ledger goes from 1.52ms/671 allocs to 1.43ms/264 allocs and `ExtractLedgerTxParts` from 443µs/17 allocs to 430µs/9 allocs.
