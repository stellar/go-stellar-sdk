...
### New Features
* Add `BufferedStorageBackendConfig.BufferBytes`, a byte bound on the download prefetch pipeline. `BufferSize` bounds it in objects, but object size varies ~700x across pubnet history, so an object count says nothing about memory. Zero (the default) disables it, leaving dispatch behavior unchanged. Separately and for all callers, the compressed-buffer pool is now sized by `NumWorkers` rather than `BufferSize`: it only ever discards undersized buffers, so over a range spanning object growth it accumulated `BufferSize` tip-sized buffers. Backfilling pubnet tip ranges in stellar-rpc, peak RSS fell from 25.0 GB to 5.9 GB at unchanged throughput. [5988](https://github.com/stellar/go-stellar-sdk/pull/5988), [stellar-rpc#895](https://github.com/stellar/stellar-rpc/issues/895).
