...
### New Features
* Add `BufferedStorageBackendConfig.BufferBytes`, a byte cap on download prefetch. `BufferSize` bounds the pipeline in objects, but object size varies ~700x across pubnet history, so a count says nothing about memory. `BufferBytes` converts the cap into a depth. Zero (the default) disables it. The compressed-buffer pool is also now sized by `NumWorkers` rather than `BufferSize`, for all callers. Backfilling pubnet tip ranges in stellar-rpc, peak RSS fell from 25.0 GB to 5.8 GB at unchanged throughput. [5988](https://github.com/stellar/go-stellar-sdk/pull/5988), [stellar-rpc#895](https://github.com/stellar/stellar-rpc/issues/895).
