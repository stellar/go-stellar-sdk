...
### New Features
* Add `BufferedStorageBackendConfig.BufferBytes`, a byte bound on the download prefetch pipeline. `BufferSize` bounds it in *objects*, but an object's size is a property of the data rather than of the configuration — pubnet objects grow roughly 700x between early history and the tip, and a datastore packing several ledgers per object multiplies that again — so one depth setting costs megabytes over one range and gigabytes over another. `BufferBytes` meters the whole pipeline rather than just what has already arrived: buffered payload at the capacity of the pooled buffer holding it (buffers are over-allocated and reused, and `Get` only discards undersized ones, so each ratchets toward the largest object it has served — capacity runs ~2.3x payload on pubnet tip objects, and charging length would under-report by that factor), in-flight downloads at their reported size, and dispatched-but-unstarted tasks at the running mean; whichever of the two bounds binds first applies, and zero (the default) disables it for the previous behavior. Backfilling pubnet tip ranges in stellar-rpc, peak RSS fell from 25.0 GB to 5.9 GB at unchanged throughput. [stellar-rpc#895](https://github.com/stellar/stellar-rpc/issues/895).
