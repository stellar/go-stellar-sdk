...
* Add `ExtractFees` (**experimental** — signatures may still change), a zero-copy, view-based fee extractor returning one ledger's fee observations (`LedgerFees`) split into classic per-op fees and Soroban inclusion fees — the buckets stellar-rpc's `getFeeStats` windows consume. The classification replicates stellar-rpc v1's `FeeWindows.IngestFees` exactly (validated by a verbatim test-only port of that walk plus cross-checks against the parsed `LedgerTransaction` fee helpers, synthetic fixtures across LCM V0/V1/V2 × `TransactionMeta` V0–V4 × envelope types, and real-ledger equivalence tests). [5966](https://github.com/stellar/go-stellar-sdk/pull/5966).
* Add zero-copy XDR view extractors (**experimental** — signatures may still change) — the view-based twins of the parsed transaction path, reading directly off the generated `*View` accessors without `UnmarshalBinary`: `ExtractTxHashes` (per-ledger transaction hashes in apply order), `ExtractLedgerEvents` (per-transaction contract events + hash from one `TxProcessing` walk, as raw wire bytes), and the zero-copy `LedgerTransactionView` read path `LedgerTransactionViewByHash` / `LedgerTransactionViewRange` (the view parallel of `LedgerTransaction` / `LedgerTransactionReader`). `network.TransactionViewHasher` computes transaction hashes straight from envelope view bytes. Validated wire-identical against the parsed path across LCM V0/V1/V2 and `TransactionMeta` V0–V4, plus real-ledger equivalence tests and benchmarks against the eager-decode path on `ledger_58752000`. [5949](https://github.com/stellar/go-stellar-sdk/pull/5949).
