...
* Add zero-copy XDR view extractors — the view-based twins of the parsed transaction path, reading directly off the generated `*View` accessors without `UnmarshalBinary`: `DispatchLedgerCloseMetaView` (per-version `LedgerCloseMeta` navigation: `Header`, `TxProcessing`, `Envelopes`, `TxProcessingHash`), `TransactionEventsView` with `TransactionEventsFromMeta` / `DiagnosticEventsFromMeta` (the view analog of `LedgerTransaction.GetTransactionEvents` / `GetDiagnosticEvents`), and the zero-copy `TransactionView` read path `TransactionViewByHash` / `TransactionViewRange` (the view analog of `LedgerTransaction` / `LedgerTransactionReader`). Validated wire-identical against the parsed path across LCM V0/V1/V2 and `TransactionMeta` V0–V4. [5949](https://github.com/stellar/go-stellar-sdk/pull/5949).
