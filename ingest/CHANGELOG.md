...
* Add `ExtractFees` (**experimental** — signatures may still change), a zero-copy, view-based fee extractor returning one ledger's fee observations (`LedgerFees`) split into classic per-op fees and Soroban inclusion fees — the buckets stellar-rpc's `getFeeStats` windows consume. The fee classification is exactly stellar-rpc v1's `FeeWindows.IngestFees`, covered by table-driven fixtures across LCM V0/V1/V2 × `TransactionMeta` V0–V4 × envelope types and by a real-pubnet-ledger test. [5966](https://github.com/stellar/go-stellar-sdk/pull/5966).
