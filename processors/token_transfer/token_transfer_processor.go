...
type EventsProcessor struct {
	...
	// Instantiate TTP with this flag only if you know that your ledgerCloseMeta has unified events in TxMeta
	// i.e, the ledgers were generated by stellar-core when the EMIT_CLASSIC_EVENTS and BACKFILL_STELLAR_ASSET_EVENTS flags were set to true
	readFromUnifiedEventsStream bool
}

type EventsProcessorOption func(*EventsProcessor)

func WithUnifiedEventsStreamEnabled() EventsProcessorOption {
	return func(ep *EventsProcessor) {
		ep.readFromUnifiedEventsStream = true
	}
}
...
func NewEventsProcessorForUnifiedEvents(passphrase string) *EventsProcessor {
	return NewEventsProcessor(passphrase, WithUnifiedEventsStreamEnabled())
}
...
func (p *EventsProcessor) EventsFromLedger(lcm xdr.LedgerCloseMeta) ([]*TokenTransferEvent, error) {
	...
	isProtocol23Plus := lcm.LedgerHeaderHistoryEntry().Header.LedgerVersion >= 23
	...
	/*
		...
		Protocol 23 onwards,  the following represents chronological ordering of events in a ledger
			- FeeEvents from all Transactions upfront
			- For each transaction
				events from each operation
			- FeeRefund for all transactions at the very end
	...
	*/
	...
	var feeRefundEvents []*TokenTransferEvent
	...
	for {
		...
		if len(txEvents.FeeEvents) == 0 || len(txEvents.FeeEvents) > 2 {
			return nil, fmt.Errorf("invalid feeEvents found for transaction: %v, feeEvents found: %v",
				tx.Hash.HexString(), len(txEvents.FeeEvents))
		}

		// Always collect fee events upfront
		feeEvents = append(feeEvents, txEvents.FeeEvents[0])
		operationEvents = append(operationEvents, txEvents.OperationEvents...)

		// Handle fee refunds based on protocol version
		if len(txEvents.FeeEvents) == 2 {
			if isProtocol23Plus {
				// Protocol 23+: collect all refunds for the end
				feeRefundEvents = append(feeRefundEvents, txEvents.FeeEvents[1])
			} else {
				// Pre-protocol 23: add refund immediately after operation events
				operationEvents = append(operationEvents, txEvents.FeeEvents[1])
			}
		}
	}

	// Assemble final event order based on protocol
	return p.assembleEventOrder(feeEvents, operationEvents, feeRefundEvents, isProtocol23Plus), nil
}
...
func (p *EventsProcessor) EventsFromOperation(tx ingest.LedgerTransaction, opIndex uint32, op xdr.Operation, opResult xdr.OperationResult) ([]*TokenTransferEvent, error) {
	...
	case xdr.OperationTypeInvokeHostFunction:
		events, err = p.contractEventsFromOperation(tx, opIndex)
	default:
		return nil, nil
	}
...
func (p *EventsProcessor) contractEventsFromOperation(tx ingest.LedgerTransaction, opIndex uint32) ([]*TokenTransferEvent, error) {
	contractEvents, err := tx.GetContractEventsForOperation(opIndex)
	if err != nil {
...
*/
func (p *EventsProcessor) mintOrBurnOrTransferEvent(tx ingest.LedgerTransaction, opIndex *uint32, asset xdr.Asset, fromStrkey string, toStrkey string, amt string, includeMuxInfo bool) (*TokenTransferEvent, error) {
...
func (p *EventsProcessor) paymentEvents(tx ingest.LedgerTransaction, opIndex uint32, op xdr.Operation) ([]*TokenTransferEvent, error) {
	...
	event, err := p.mintOrBurnOrTransferEvent(tx, &opIndex, paymentOp.Asset, opSrcAcc.Address(), destAcc.Address(), amt, true)
	if err != nil {
		return nil, err
	}
...
func (p *EventsProcessor) createClaimableBalanceEvents(tx ingest.LedgerTransaction, opIndex uint32, op xdr.Operation, result xdr.OperationResult) ([]*TokenTransferEvent, error) {
	...
	event, err := p.mintOrBurnOrTransferEvent(tx, &opIndex, createCbOp.Asset, opSrcAcc.Address(), cbIdToStrkey(cbId), amount.String64Raw(createCbOp.Amount), false)
	if err != nil {
		return nil, err
	}
	return []*TokenTransferEvent{event}, nil
...
func (p *EventsProcessor) claimClaimableBalanceEvents(tx ingest.LedgerTransaction, opIndex uint32, op xdr.Operation) ([]*TokenTransferEvent, error) {
	...
	event, err := p.mintOrBurnOrTransferEvent(tx, &opIndex, cb.Asset, cbIdToStrkey(cbId), opSrcAcc.Address(), amount.String64Raw(cb.Amount), true)
	if err != nil {
		return nil, err
	}
	return []*TokenTransferEvent{event}, nil
...
func (p *EventsProcessor) liquidityPoolDepositEvents(tx ingest.LedgerTransaction, opIndex uint32, op xdr.Operation) ([]*TokenTransferEvent, error) {
	...
	var events []*TokenTransferEvent
	event, err := p.mintOrBurnOrTransferEvent(tx, &opIndex, assetA, opSrcAcc.Address(), lpIdToStrkey(delta.liquidityPoolId), amount.String64Raw(amtA), false)
	if err != nil {
		return nil, err
	}
	events = append(events, event)

	event, err = p.mintOrBurnOrTransferEvent(tx, &opIndex, assetB, opSrcAcc.Address(), lpIdToStrkey(delta.liquidityPoolId), amount.String64Raw(amtB), false)
	if err != nil {
		return nil, err
	}
	events = append(events, event)
...
func (p *EventsProcessor) liquidityPoolWithdrawEvents(tx ingest.LedgerTransaction, opIndex uint32, op xdr.Operation) ([]*TokenTransferEvent, error) {
	...
	*/
	...
	var events []*TokenTransferEvent
	event, err := p.mintOrBurnOrTransferEvent(tx, &opIndex, assetA, lpIdToStrkey(delta.liquidityPoolId), opSrcAcc.Address(), amount.String64Raw(amtA), true)
	if err != nil {
		return nil, err
	}
	events = append(events, event)

	event, err = p.mintOrBurnOrTransferEvent(tx, &opIndex, assetB, lpIdToStrkey(delta.liquidityPoolId), opSrcAcc.Address(), amount.String64Raw(amtB), true)
	if err != nil {
		return nil, err
	}
	events = append(events, event)
...
func (p *EventsProcessor) generateEventsFromClaimAtoms(tx ingest.LedgerTransaction, opIndex uint32, opSrcAcc xdr.MuxedAccount, claims []xdr.ClaimAtom) ([]*TokenTransferEvent, error) {
	...
	for _, claim := range claims {
		...
		ev1, err := p.mintOrBurnOrTransferEvent(tx, &opIndex, claim.AssetSold(), seller, operationSource, amount.String64Raw(claim.AmountSold()), false)
		if err != nil {
			return nil, err
		}
		ev2, err := p.mintOrBurnOrTransferEvent(tx, &opIndex, claim.AssetBought(), operationSource, seller, amount.String64Raw(claim.AmountBought()), false)
		if err != nil {
			return nil, err
		}
...
func (p *EventsProcessor) pathPaymentStrictSendEvents(tx ingest.LedgerTransaction, opIndex uint32, op xdr.Operation, result xdr.OperationResult) ([]*TokenTransferEvent, error) {
	...
	finalEvent, err := p.mintOrBurnOrTransferEvent(tx, &opIndex, strictSendOp.DestAsset, opSrcAcc.Address(), strictSendOp.Destination.Address(), amount.String64Raw(strictSendResult.DestAmount()), true)
	if err != nil {
		return nil, err
	}
...
func (p *EventsProcessor) pathPaymentStrictReceiveEvents(tx ingest.LedgerTransaction, opIndex uint32, op xdr.Operation, result xdr.OperationResult) ([]*TokenTransferEvent, error) {
	...
	// Generate one final event indicating the amount that the destination received in terms of destination asset
	finalEvent, err := p.mintOrBurnOrTransferEvent(tx, &opIndex, strictReceiveOp.DestAsset, opSrcAcc.Address(), strictReceiveOp.Destination.Address(), amount.String64Raw(strictReceiveOp.DestAmount), true)
	if err != nil {
		return nil, err
	}
	events = append(events, finalEvent)
